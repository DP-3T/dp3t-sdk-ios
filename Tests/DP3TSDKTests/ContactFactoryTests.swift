...
final class ContactFactoryTests: XCTestCase {
    func testShouldGrouping() {
        ...
        XCTAssertEqual(contacts.first?.handshakes , handshakes)
    ...
    func testShouldNotGroup(){
        ...
        let handshake1 = HandshakeModel(identifier: 0, timestamp: Date(), ephID: token1, TXPowerlevel: nil, RSSI: nil, knownCaseId: nil)
        let handshake2 = HandshakeModel(identifier: 1, timestamp: Date().addingTimeInterval(1), ephID: token2, TXPowerlevel: nil, RSSI: nil, knownCaseId: nil)
        let handshakes = [handshake1, handshake2]
        let contacts = ContactFactory.contacts(from: handshakes, contactThreshold: 0)
